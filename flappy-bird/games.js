(function () {
  ...
  function postEvent (eventType, callback, eventData) {
    clg(`postEvent ${eventType} ${eventData} `)
    if (!callback) {
      callback = function () {};
    }
    ...
    if (window.TelegramWebviewProxy !== undefined) {
      clg(`window.TelegramWebviewProxy !== undefined`);
      clg(`window.TelegramWebviewProxy ${JSON.stringify(TelegramWebviewProxy.postEvent,null,2)}`);
      TelegramWebviewProxy.postEvent(eventType, JSON.stringify(eventData));
      callback("error: sample error");
    }
    else if (window.external && 'notify' in window.external) {
      clg(`window.external && 'notify' in window.external`)
      ...
      try {
        ...
        trustedTarget = '*';
      clg(`${JSON.stringify({eventType: eventType, eventData: eventData})}, ${trustedTarget}`)
